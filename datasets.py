...
def create_dataloader(path, imgsz, batch_size, stride, hyp=None, augment=False, cache=False, pad=0.0,
                      rank=-1, world_size=1, workers=4, image_weights=False, quad=False, prefix='', tidl_load=False, kpt_label=False):
    # Make sure only the first process in DDP process the dataset first, and the following others can use the cache
    with torch_distributed_zero_first(rank):
        dataset = YOLOKPDataset(path, imgsz, batch_size,
    ...
    nw = min([os.cpu_count() // world_size, batch_size if batch_size > 1 else 0, workers[0]])  # number of workers
    ...
    dataloader = loader(dataset,
                        ...
                        sampler=sampler,
                        pin_memory=True,
                        collate_fn=YOLOKPDataset.collate_fn4 if quad else YOLOKPDataset.collate_fn)
...
class YOLOKPDataset(Dataset):
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, image_weights=False,
        ...
        # Reference: https://github.com/TexasInstruments/edgeai-yolov5/blob/695436ca0f5e00c4283ea3d1dd45b5437ca0dd47/utils/datasets.py#L349
        # --------------------------------------------
