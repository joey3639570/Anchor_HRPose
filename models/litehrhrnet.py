...
def channel_shuffle(x, groups):
    """Channel Shuffle operation. (Copied from mmpose)
...
class depthwise_separable_conv(nn.Module):
    def __init__(self, nin, nout, kernel_size = 3, padding = 1, bias=False):
        ...
        self.bn_d = nn.BatchNorm2d(nin)
        self.bn_p = nn.BatchNorm2d(nout)
...
class CrossResolutionWeighting(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv1 = nn.Conv2d(total_channel, int(total_channel / ratio), kernel_size=1, stride=1, bias=False)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = nn.Conv2d(int(total_channel / ratio), total_channel, kernel_size=1, stride=1, bias=False)
        self.sigmoid = nn.Sigmoid()
...
class ConditionalChannelWeighting(nn.Module):
    ...
    def __init__(self,
        ...
        #print("CrossResolutionWeighting, branch_channels", branch_channels)
...
class Stem(nn.Module):
    ...
    def __init__(self,
        ...
        self.branch1 = nn.Sequential(
            nn.Conv2d(branch_channels, branch_channels, kernel_size=3, stride=2, padding=1, groups=branch_channels, bias=False),
    ...
    def __init__(self, in_channels, conv_cfg=None, norm_cfg=dict(type='BN')):
        ...
        for i in range(num_branchs):
            if i != num_branchs - 1:
                projects.append(
                    nn.Sequential(
                        depthwise_separable_conv(self.in_channels[i], self.in_channels[i + 1], kernel_size = 3,  padding = 1, bias=False),
                        nn.ReLU(inplace=True)
                        )
                    )
            else:
                projects.append(
                    nn.Sequential(
                        depthwise_separable_conv(self.in_channels[i], self.in_channels[i], kernel_size = 3,  padding = 1, bias=False),
                        nn.ReLU(inplace=True)
                        )
                    )
...
class LiteHRModule(nn.Module):
    ...
    def _make_weighting_blocks(self, num_blocks, reduce_ratio, stride=1):
        ...
        for i in range(num_blocks):
            #print("_make_weighting_blocks : ", self.in_channels)
            layers.append(
